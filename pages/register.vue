        ...
        <form class="form" @submit.prevent="nextOrSubmit" novalidate>
          <template v-if="step === 0">
            ...
            <div class="grid resp">
              <div class="stack">
                ...
                <div class="pw-meter" aria-live="polite">
                  <div class="pw-meter__bar">
                    <span
                      class="pw-meter__fill"
                      :style="{ width: strengthPercent }"
                    ></span>
                  </div>
                  <div class="pw-meter__legend">
                    Fortaleza:
                    <strong :class="'pw-' + strengthLabel.toLowerCase()">{{
                      strengthLabel
                    }}</strong>
                  </div>
                  <ul class="pw-reqs">
              ...
              <div class="stack">
                ...
                <small class="help" v-if="password2 && !pwdMatch"
          ...
          <template v-else-if="step === 1">
            <div class="grid resp">
              ...
              <div class="stack">
                <label class="label" for="paternal_surname"
              ...
              <div class="stack">
                <label class="label" for="maternal_surname"
              ...
              <div class="stack col-span-2">
                <label class="label" for="emergency_phone"
                  >Teléfono de emergencia (opcional)</label
                >
                <input
                  id="emergency_phone"
          ...
          <template v-else-if="step === 2">
            <div class="grid resp">
              <div class="stack">
                ...
                <select
                  id="type_user_id"
                  v-model.number="form.type_user_id"
                  class="input"
                  required
                >
            ...
            <div
              v-if="form.provenance === 'uttecam' && form.type_user_id === 2"
              class="grid resp"
            >
              <div class="stack">
                <label class="label" for="educational_program"
                  >Programa educativo</label
                >
                <select
            ...
            <div
              v-if="form.provenance === 'uttecam' && form.type_user_id === 1"
              class="grid resp"
            >
              <div class="stack">
                <label class="label" for="matricula">Matrícula</label>
            ...
            >
              <div class="stack col-span-2">
                <label class="label" for="provenance_other"
                  >Empresa o Universidad</label
                >
                <input
                  id="provenance_other"
          ...
          <template v-else-if="step === 3">
            ...
            <div class="checkline">
              ...
              <label for="terms"
                >Acepto los términos y aviso de privacidad</label
              >
            </div>
          </template>
...
const pwdMatch = computed(
  () =>
    password2.value === form.value.password_user && password2.value.length > 0
);
const strengthScore = computed(
  () => Object.values(reqs.value).filter(Boolean).length
);
...
const isSpeakerLike = computed(
  () => form.value.type_user_id === 4 || form.value.type_user_id === 5
);
  ...
  if (step.value === 2) {
    ...
    if (form.value.provenance === "uttecam") {
      ...
      if (form.value.type_user_id === 1) {
        // Estudiante
        const okGrade =
          !form.value.grade || String(form.value.grade).length <= 2;
        const okGroup =
          !form.value.group_user || String(form.value.group_user).length <= 1;
        const okMat =
          !!form.value.matricula && String(form.value.matricula).length <= 8;
    ...
    // Para externos
    if (
      form.value.provenance === "otro" &&
      (form.value.type_user_id === 1 ||
        form.value.type_user_id === 2 ||
        form.value.type_user_id === 3)
    ) {
      return (
        !!form.value.provenance &&
        form.value.provenance !== "uttecam" &&
        form.value.provenance !== "otro"
      );
    }
...
const reasons = computed(() => {
  ...
  if (step.value === 2) {
    ...
    if (form.value.provenance === "uttecam") {
      if (form.value.type_user_id === 2) {
        if (!form.value.educational_program)
          r.push("Falta programa educativo (Docente)");
        if (!form.value.matricula) r.push("Falta matrícula (Docente)");
      }
      if (form.value.type_user_id === 1) {
        if (!form.value.educational_program)
          r.push("Falta programa educativo (Estudiante)");
        if (!form.value.matricula)
          r.push("Falta matrícula (Estudiante, máx 8)");
...
function nextOrSubmit() {
  ...
  if (step.value < steps.length - 1) {
    if (!canProceed.value) {
      alert(
        "Revisa los campos requeridos del paso actual: " +
          reasons.value.join(", ")
      );
      return;
    }
...
async function submitRegister() {
  ...
  try {
    const payload = {
      ...
      provenance:
        form.value.provenance === "otro" ? "Otro" : form.value.provenance,
      ...
      workshop_id: form.value.workshop_id
        ? Number(form.value.workshop_id)
        : null,
      type_user_id: form.value.type_user_id
        ? Number(form.value.type_user_id)
        : null, // BigInt?
