...
export function useAdminUsers() {
  ...
  function buildBackendFilter() {
    ...
    if (fType.value && fType.value !== 'Todos') {
      filters.push(`type:${fType.value}`);
    }

    if (fState.value && fState.value !== 'Todos') {
      const stateMap = {
        'Activo': 'active',
        'Inactivo': 'inactive',
      ...
      filters.push(`status:${stateMap[fState.value] || fState.value}`);
    }

    if (fPay.value && fPay.value !== 'Todos') {
      filters.push(`payment:${fPay.value === 'Pagado' ? 'true' : 'false'}`);
    }

    // 👈 Cambio clave: devolver undefined en lugar de "Todos"
  ...
  async function setActivation(userId, activate, reason, force) {
    const run = () => AdminUsersApi.setActivation(
      userId,
      {
        ...
        reason: reason ?? null,
        force: !!force,
        status_event: !!activate // 👈 SIEMPRE enviar status_event
      },
      { timeout: 15000 }
    )
    ...
    if (idx !== -1) {
      ...
      const status_event = (res && 'status_event' in res) ? !!res.status_event : !!activate // 👈 Usar activate como fallback

      users.value[idx] = {
        ...prev,
        eventEnabled,
        status_event, // 👈 Asegurar que se actualice
  ...
  // En tu composables/admin/use-users.js - actualiza la función bulkActivate
  async function bulkActivate(activate) {
    ...
    try {
      const result = await AdminUsersApi.bulkActivation({
        ids,
        activate,
        force: true,
        status_event: activate // 👈 ENVIAR status_event también
      }, { timeout: 60000 })
      ...
      for (const u of users.value) {
        ...
        if (r) {
          ...
          u.status_event = typeof r.status_event === 'boolean' ? r.status_event : !!activate // 👈 ACTUALIZAR status_event
          ok++
        }
      }
    ...
    try {
      for (const id of ids) {
        try {
          // 👈 ACTUALIZACIÓN: Enviar status_event también en activación individual
  ...
  async function bulkActivateUI(flag) {
    ...
    // 👈 ACTUALIZAR AMBOS CAMPOS de forma optimista
    snapshots.forEach(({ row }) => {
      row.eventEnabled = flag
      row.status_event = flag // 👈 ESTA ES LA CLAVE
    })
