<template>
  <section class="workshops-section" id="workshops">
      ...
      >
        <div class="payment-modal">
          ...
          <div class="modal-body">
            ...
            <p class="modal-note">
              Una vez que tu pago sea verificado, podrás inscribirte en los talleres.
            </p>
          </div>
          <div class="modal-footer">
            <button @click="showPaymentModal = false" class="btn btn-primary">
              Entendido
            </button>
          </div>
        </div>
    ...
    <div v-else-if="error && !hasWorkshops" class="error-state" role="alert">
      ...
      <p>{{ error }}</p>
      <button @click="loadWorkshops" class="btn retry-btn">
        Reintentar
      </button>
    </div>
    ...
    <div v-else>
      <header class="workshop-header">
        ...
        <div v-if="isAuthenticated" class="user-workshop-status">
          ...
          <div v-else class="user-selection-info">
            ...
            <p class="selection-subtitle">
              <span v-if="hasPayment">✅ Tu pago está verificado - Puedes inscribirte</span>
              <span v-else>⏳ Espera la verificación de tu pago para inscribirte</span>
            </p>
          </div>
        </div>
        ...
        <div class="workshop-filters">
          ...
          >
            Todos los talleres ({{ workshops.length }})
          </button>
          <button 
          ...
          >
            Con disponibilidad ({{ availableWorkshops.length }})
          </button>
        </div>
      </header>
      ...
      <div v-if="hasWorkshops" class="workshop-grid">
        <article
          v-for="workshop in displayedWorkshops"
          :key="workshop.id"
          class="workshop-card"
        ...
        >
          <div class="workshop-card-header" :style="{ background: workshop.gradient }">
            <div class="workshop-header-top">
              <div class="workshop-header-icon-wrap" :class="levelClass(workshop)">
                <component :is="workshop.icon" class="workshop-header-icon" />
              </div>
              <div class="workshop-badges">
                <span class="workshop-badge" :class="levelClass(workshop)">{{
                  workshop.level
                }}</span>
          ...
          <div class="workshop-card-body">
            ...
            <div class="workshop-spots">
              <div class="spots-info">
                <span class="spots-text">
                  ...
                  <span v-if="workshop.available_spots > 0" class="available-text">
                    ({{ workshop.available_spots }} disponibles)
                  </span>
                  <span v-else-if="workshop.spots_max > 0" class="full-text">
                    (Cupo lleno)
                  </span>
                </span>
            ...
            <!-- Botón de inscripción según el estado -->
...
// Talleres a mostrar según el filtro
const displayedWorkshops = computed(() => {
